...
export function rpsls(locz) {
	...
	if(locz == "none")
    {
        ...
        if(typeof locz != "string")
    {
                var finalVal = { player: "(rock|paper|scissors|lizard|spock)", opponent: opponent, result: finalResult };
                return finalVal;
    }
    
	
	let inputLC = locz.toString().toLowerCase();
...
export function rps(locz) {
	...
	if(locz == "none")
    {
    	...
    	if(typeof locz != "string")
    {
		var finalVal = { player: "(rock|paper|scissors)", opponent: opponent, result: finalResult };
	        return finalVal;
    }
	let inputLC = locz.toString().toLowerCase();
	var finalResult = winChecker(inputLC, opponent)
